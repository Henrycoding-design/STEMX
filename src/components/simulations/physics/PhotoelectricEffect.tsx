...
export default function PhotoelectricEffect() {
  ...
  const points = useMemo(() => [
    ...
    { frequency: 470, potential: Math.max(0, 0.004136 * 470 - workFunction) },
    { frequency: 540, potential: Math.max(0, 0.004136 * 540 - workFunction) },
    { frequency: 610, potential: Math.max(0, 0.004136 * 610 - workFunction) },
    { frequency: 680, potential: Math.max(0, 0.004136 * 680 - workFunction) }
  ], [workFunction]);
  ...
  const chartX = (value: number) => 42 + ((value - 400) / 280) * 300;
      ...
      <div className="flex-1 grid lg:grid-cols-3 gap-6 min-h-0">
        <div className="lg:col-span-2 bg-slate-900 border border-slate-800 rounded-xl p-5 flex flex-col gap-4 min-h-[520px]">
          <div className="flex items-center justify-between text-xs font-mono text-slate-400">
            <span className="flex items-center gap-2"><span className="w-2.5 h-2.5 rounded-full bg-yellow-300" />Photon stream</span>
            <span className={emitted ? "text-emerald-400" : "text-rose-400"}>{emitted ? "PHOTOELECTRONS EMITTED" : "BELOW THRESHOLD"}</span>
          </div>
          <svg viewBox="0 0 760 270" className="w-full flex-1 min-h-[250px] rounded-lg bg-slate-950" role="img" aria-label="Photoelectric effect visualization">
            ...
            <text x="24" y="13" fill="#e2e8f0" fontSize="10" fontFamily="monospace">COLLECTOR PLATE · Vₐₚₚ = {appliedVoltage.toFixed(1)} V</text>
          ...
          <div className="grid sm:grid-cols-2 xl:grid-cols-4 gap-3">
            <Metric label="Frequency" value={`${frequencyTHz.toFixed(1)} THz`} />
            <Metric label="Stopping potential" value={`${stoppingPotential.toFixed(2)} V`} />
            <Metric label="Threshold wavelength" value={`${thresholdWavelength.toFixed(0)} nm`} />
            <Metric label="Photocurrent" value={`${photocurrent.toFixed(2)} μA`} />
          </div>

          <div className="rounded-lg bg-slate-950 border border-slate-800 p-3">
            <div className="flex justify-between items-center mb-2"><span className="text-xs font-bold text-slate-300">Stopping Potential Vₛ vs Frequency f</span><span className="text-[10px] text-emerald-400">slope ≈ h/e</span></div>
            <svg viewBox="0 0 370 190" className="w-full h-44">
              ...
              {[0, 1, 2, 3].map(value => <g key={value}><line x1="42" y1={166 - value * 42} x2="342" y2={166 - value * 42} stroke="#1e293b" /><text x="5" y={170 - value * 42} fill="#64748b" fontSize="10">{(maxPotential * value / 3).toFixed(1)}</text></g>)}
              ...
              <text x="145" y="187" fill="#94a3b8" fontSize="10">frequency (THz)</text><text x="5" y="23" fill="#94a3b8" fontSize="10">Vₛ (V)</text>
            </svg>
            <p className="text-[11px] text-slate-400">Experimental estimate: <span className="text-emerald-400 font-mono">h = {estimatedH ? `${estimatedH.toFixed(4)} eV/THz` : "—"}</span> (accepted value: 0.00414 eV/THz)</p>
          </div>
        </div>

        <aside className="space-y-6 overflow-y-auto pr-2">
          <section className="bg-slate-900 border border-slate-800 rounded-xl p-6">
            <h3 className="font-bold text-slate-100 mb-5 flex items-center gap-2"><Zap className="w-4 h-4 text-yellow-300" />Light source</h3>
            <Slider label="Wavelength" value={`${wavelength} nm`} min={250} max={750} valueNumber={wavelength} onChange={setWavelength} />
            <Slider label="Intensity" value={`${intensity}%`} min={10} max={100} valueNumber={intensity} onChange={setIntensity} />
            <p className="text-[11px] text-slate-500 mt-3">Frequency changes electron energy; intensity changes the emission rate.</p>
          </section>
          <section className="bg-slate-900 border border-slate-800 rounded-xl p-6">
            <h3 className="font-bold text-slate-100 mb-4 flex items-center gap-2"><Atom className="w-4 h-4 text-indigo-400" />Target metal</h3>
            ...
            <div className="mt-5 flex justify-between text-xs"><span className="text-slate-400">Work function</span><span className="font-mono text-indigo-300">{workFunction.toFixed(2)} eV</span></div>
          </section>
          <section className="bg-slate-900 border border-slate-800 rounded-xl p-6">
            <h3 className="font-bold text-slate-100 mb-5">Retarding circuit</h3>
            <Slider label="Applied Voltage" value={`${appliedVoltage.toFixed(1)} V`} min={-5} max={5} step={0.1} valueNumber={appliedVoltage} onChange={setAppliedVoltage} />
            <div className="flex justify-between text-xs"><span className="text-slate-400">Photocurrent</span><span className={photocurrent > 0 ? "text-emerald-400 font-mono" : "text-rose-400 font-mono"}>{photocurrent.toFixed(2)} μA</span></div>
            <p className="text-[11px] text-slate-500 mt-3">At Vₐₚₚ ≤ −Vₛ, electrons reverse before reaching the collector.</p>
          </section>
          <section className="bg-slate-900 border border-slate-800 rounded-xl p-6">
            <h3 className="font-bold text-slate-100 mb-3">Einstein’s equation</h3>
            <p className="font-mono text-indigo-300 text-sm">Kₘₐₓ = hf − W₀</p>
            <p className="text-xs text-slate-400 mt-3">Threshold: λ₀ = {thresholdWavelength.toFixed(0)} nm. {emitted ? "Photons exceed the work function." : "Increase frequency to eject electrons."}</p>
            <button onClick={() => { setWavelength(450); setIntensity(65); setMetal("Sodium"); setAppliedVoltage(0); }} className="mt-4 inline-flex items-center text-xs text-slate-400 hover:text-white"><RotateCcw className="w-3 h-3 mr-1" />Reset experiment</button>
          </section>
        </aside>
      </div>
